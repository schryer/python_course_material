...
**Material by Paul Wilson, Milad Fatenejad, Sasha Wood, Radhika Khetani, and David Schryer**

# What is the shell? How do I access the shell?

The *shell* is a program that provides a command line interface
which allows you to run other programs.  This allows users to 
control their computer using a keyboard instead of using graphical 
...
Login or create an account on GitHub.
...
2.  The shell is a powerful way of interacting with your computer. 
    
3.  Using the shell will greatly expand the range of tasks you can 
    accomplish with your computer. 

4.  You will also be able to perform many tasks more efficiently.

The most common shell (and the one we will use) is the Bourne-Again
SHell (bash).  Even if bash is not the default shell, it is usually
installed on most systems and can be started by typing `bash` in the
terminal. 
...
directory) is used to move around. Let's move into the `python_course_material`
...
Programs that are run from the shell may have many arguments. To
...
`/home/USER-NAME/python_course_material`. You can chain these
together, so:

    ls ../../

prints the contents of `/home/USER-NAME` which is your home
...
`python_course_material`. Now enter:
...
then `Ctrl-d`. You should is the lines:

    are
    good

printed back at you. The `Ctrl-d` keyboard shortcut inserts an
...
really do anything all that impressive. HOWEVER, when you start chaining
...
3.  Enter the command: `python generate_data.py` You should see that the
